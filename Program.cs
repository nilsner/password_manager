...
namespace Code_off
{
    class Program
    {
        public static void Main(string[] args)
        {
            ...
            args = new string[] { "secret", "ClientInfo3.json" };
            ...
            else
            {
                switch (args[0]) //ALLA METODER MÅSTE TA EMOT OCH ANVÄNDA args[1] och args[2]. Innan det här körs ska man kontrollera att det ens finns tillräckligt med inputs och i korrekt form
                {
                    ...
                    case "set": //KLAR FÖRRUTOM G, vad är G? två varianter som på "get", en utan "-g" och en med "-g". Ska försöka göra ChangeServicePsw(), om inte det värdet finns ska WriteToServer().
                        if (args.Length > 4)
                        {
                            Console.WriteLine("Something went wrong, try again");
                        }
                    ...
                    case "login":
                        ...
                        {
                            //RÄTT INLOGG MEN VAD HÄNDER SEN???
                        }
                        else
                        {
